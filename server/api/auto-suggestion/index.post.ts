...
export default defineEventHandler(async event => {
	...
	try {
		...
		if (visitingSpeakers.length === 0) {
			...
			return await getFallbackSuggestion(db, body.excludedSpeakerIds)
		}
		...
		const talkPoolQuery = db
			.select({
				...
				lastGivenDate: sql<Date | null>`MAX(${scheduledPublicTalks.date})`.as(
					"last_given_date"
				),
			})
			.from(speakerTalks)
			...
			.where(
				and(
					...
					inArray(speakers.id, visitingSpeakerIds)
				)
			)
		...
		const speakerLastTalkQuery = db
			.select({
				...
				lastTalkDate: sql<Date | null>`MAX(${scheduledPublicTalks.date})`.as(
					"last_talk_date"
				),
			})
			.from(speakers)
 ...
 */
async function getFallbackSuggestion(
