...
type ChipColor = "error" | "info" | "primary" | "secondary" | "success" | "warning" | "neutral"
...
export function useCalendarChipColor() {
   ...
   *
   * Color logic:
   * - primary (blue): Special date (e.g., Circuit Overseer visit)
   ...
   * @param specialDates - Optional array of unix timestamps (seconds) of special dates
  ...
  function getChipColor(
    ...
    // Check if it's a special date (e.g., Circuit Overseer visit)
    const isSpecial = specialDates.some(timestamp => {
      const specialDate = dayjs.unix(timestamp).utc().toDate()
      return isSameDay(dateToCheck, specialDate)
    })
    if (isSpecial) return "secondary"
