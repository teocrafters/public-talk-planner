<template>
  <div v-if="title || description" class="text-center mb-6">
  ...
  >
    ...
    <!-- Email -->
    <UFormField :label="t('auth.emailAddress')" name="email" required>
      <UInput 
    ...
    <UFormField :label="t('auth.congregation')" name="congregationId" required>
      <USelectMenu 
...
// Load congregations
const { data: congregations, pending: congregationsLoading } = await useFetch<Congregation[]>('/api/congregations')
